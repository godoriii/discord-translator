...
- **▶ 번역**: 지금 화면에 보이는 모든 메시지를 즉시 번역합니다 — 스크롤 위치, dwell 대기, 분당 40건 백필 제한을 모두 무시합니다.
...
- **일부 메시지가 번역 안 됨** → `.`이나 이모지만 있는 메시지, 이미 한국어 비율이 35% 이상인 메시지는 의도적으로 건너뜁니다. 그 외의 경우라면 화면 우측 하단 **▶ 번역** 버튼(또는 Alt+Shift+T)을 누르세요.
